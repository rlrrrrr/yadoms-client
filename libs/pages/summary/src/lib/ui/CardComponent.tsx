...
export function Card({ icon, label, content, color, isLoading, children }: CardProps) {
  return (
    <Container p={'xl'} className={classes.item} styles={(theme) => ({
      root: {
        boxShadow: `0px 0px 20px ${color}`, border: `1px solid ${color}`
      }
    })}>
      <Center>
        {icon}
